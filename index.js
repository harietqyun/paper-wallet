...
async function main() {
  ...
  var wallet;
  if (['btc','btg','dash','dcr','doge','ltc','mona','qtum','rdd','sct','zec'].indexOf(symbol) >= 0) {
  ...
  } else if (['sct'].indexOf(symbol) >= 0) {
    wallet = sct.generateWallet(seed, symbol);
  } else {
    console.log('Usage: paper-wallet [btc|bch|btg|dash|dcr|doge|eos|eth|etc|ltc|mona|qtum|rdd|sct|zec]');
    return;
  }
